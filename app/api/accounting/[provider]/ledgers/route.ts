...
export async function GET(
  ...
  try {
    const ledgers = await withValidToken(connection, async (token) => {
      const adapter = await getAdapter(
        provider as AccountingProvider,
        token,
        connection.externalAdminId,
      )
      return adapter.getLedgerAccounts()
    })
    ...
    return NextResponse.json({ error: 'Failed to fetch ledger accounts' }, { status: 500 })
  }
}
